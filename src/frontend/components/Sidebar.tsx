...
export default function Sidebar() {
  ...
  function isActive(href: string, exact?: boolean) {
    ...
    if (href === '') {
      // For Dashboard, only highlight if the path is exactly the root role path
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-[#1C0F08] text-white">
      ...
      <div className="p-3 border-t border-white/5 bg-black/20">
        <div className="px-3 py-3 mb-2 bg-gradient-to-br from-white/10 to-transparent rounded-xl border border-white/10 shadow-inner group">
           <div className="flex items-center gap-3">
             <div className="w-9 h-9 rounded-lg bg-primary-600 flex items-center justify-center font-black text-sm shadow-[0_4px_12px_rgba(201,168,125,0.3)] ring-1 ring-white/20 uppercase">
               {session?.username?.[0] || userRole?.[0] || 'U'}
             </div>
             <div className="flex-1 min-w-0">
               <div className="text-[12px] font-black text-white truncate tracking-tight uppercase">
        ...
        <button
          ...
          className="w-full flex items-center gap-2.5 px-3 py-2.5 text-red-400 hover:bg-rose-500/10 hover:text-rose-400 rounded-xl transition-all font-black text-[10px] uppercase tracking-widest group"
        >
